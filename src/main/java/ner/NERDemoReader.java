...
/**
 * Reads the GermEval 2014 NER Shared Task dataset. Can be used for Unit or
 * Sequence Classification.
 *
 */
public class NERDemoReader extends JCasResourceCollectionReader_ImplBase implements TCReaderSequence

{
	...
	/**
	 * Load the chunk tag to UIMA type mapping from this location instead of
	 * locating the mapping automatically.
	 */
	...
	private void convert(JCas aJCas, BufferedReader aReader) throws IOException {
		...
		while ((words = readSentence(aReader)) != null) {
			...
			for (String[] word : words) {
				...
				TextClassificationOutcome outcome = new TextClassificationOutcome(aJCas, token.getBegin(),
						token.getEnd());
	...
	private static List<String[]> readSentence(BufferedReader aReader) throws IOException {
		...
		while ((line = aReader.readLine()) != null) {
			...
			if (fields.length != 4) {
				throw new IOException("Invalid file format. Line needs to have 4 tab-separted fields.");
			}
			words.add(fields);
		}
